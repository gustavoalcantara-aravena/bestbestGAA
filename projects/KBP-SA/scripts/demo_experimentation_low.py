...
def main():
    ...
    # Generar timestamp al inicio para usar en todos los outputs
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    plots_dir = f"output/plots_low_dimensional_{timestamp}"

    # Inicializar TimeTracker en el mismo directorio que las visualizaciones
    tracker = TimeTracker(output_file="time_tracking.md", output_dir=plots_dir)

    with tracker.track("Ejecución completa del experimento"):

        # 1. Generar algoritmos para experimentar
        with tracker.track("Paso 1: Generando algoritmos GAA", num_algorithms=3):
            ...
            tracker.update_current(algorithms_generated=len(algorithms))
        ...
        with tracker.track("Paso 3: Ejecutando experimentos", total_ejecuciones=len(config.instances) * len(config.algorithms) * config.repetitions):
        ...
        with tracker.track("Paso 7: Generando visualizaciones y documentación"):
            ...
            # Usar el plots_dir y timestamp ya definidos al inicio
    ...
    **Archivos generados**:
    ...
    # Finalizar tracking
    tracker.finalize()
    print(f"\n📊 Time tracking guardado en: {tracker.output_path}")
